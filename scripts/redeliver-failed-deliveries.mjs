...
async function checkAndRedeliverWebhooks() {
    ...
    try {
        ...
        for (const guid in deliveriesByGuid) {
            ...
            const anySucceeded = deliveries.some((delivery) => delivery.status === 'OK');
            if (!anySucceeded) {
        ...
        if (error.response) {
            console.error(`Failed to check and redeliver webhooks: ${error.response.data.message}`);
        }
        console.error(error);
        throw error;
    }
...
async function fetchWebhookDeliveriesSince({ lastWebhookRedeliveryTime, app }) {
    const iterator = app.octokit.paginate.iterator('GET /app/hook/deliveries', {
        per_page: 100,
        headers: {
