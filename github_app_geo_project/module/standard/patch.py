...
class Patch(module.Module[dict[str, Any], dict[str, Any], dict[str, Any], Any]):
    ...
    async def process(
        ...
        if context.module_event_name == "workflow_job":
            ...
            run_id = event_data.workflow_job.run_id
            head_branch = event_data.workflow_job.head_branch
            is_clone = False
            try:
                ...
                workflow_run = getattr(workflow_run_response, "parsed_data", None) or getattr(
                    workflow_run_response,
                    "data",
                    None,
                )
                ...
                head_login = getattr(head_owner, "login", None) if head_owner is not None else None
