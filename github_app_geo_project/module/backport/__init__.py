...
class Backport(module.Module[configuration.BackportConfiguration, _ActionData, None, None]):
    ...
    def get_actions(self, context: module.GetActionContext) -> list[module.Action[_ActionData]]:
        ...
        if context.event_name == "pull_request":
            ...
            if event_data_pull_request.action == "closed" and "pull_request" in context.event_data:
                actions.append(
                    module.Action(_ActionData(type="pull_request"), priority=module.PRIORITY_STANDARD),
                )
            if event_data_pull_request.action == "labeled" and "pull_request" in context.event_data:
                actions.append(
                    module.Action(_ActionData(type="pull_request"), priority=module.PRIORITY_STANDARD),
                )
            return actions
