...
class _Version:
    ...
    def __cmp__(self, other: "_Version") -> int:
        ...
        Numeric versions (matching `^\\d+(\\.(\\d+))?(\\.\\d+)*$`, e.g. "20",
        "3.11", "1.2.3") are compared numerically by major then minor component;
        patch and further components are ignored. A missing minor component is
        treated as 0 (so "20" and "20.0" are considered equal).
        ...
        if match2 is None:
            return -1
        tuple1 = (int(match1.group(1)), int(match1.group(2)) if match1.group(2) is not None else 0)
        tuple2 = (int(match2.group(1)), int(match2.group(2)) if match2.group(2) is not None else 0)
        if tuple1 == tuple2:
            return 0
